...
# Count words in Complete Continental Congress Correspondence
comp_ContCorr = count('comp_ContCorr.txt')
# and the settler petitions
Petitions = count('Petitions.txt') 

# Find out which words are used more or less
diff = llr_compare(comp_ContCorr, Petitions)
ranked = sorted(diff.items(), key=lambda x: x[1])

print("\nMore in comp_ContCorr")
